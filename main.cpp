...
void saveDemo()
{
  ...
  fstream demo;
  demo.open( saveDemoFile.data(), ios::out | ios::trunc | ios::binary );
...
bool parseCmdLine(int argc, char **argv)
{
  ...
  for(int i=1; i < argc; i++)
  {
    ...
    } else if( strcmp(argv[i], "--playdemo") == 0 )
    {
      ...
      cout << "Loading demo '" << argv[i] << "'..."<<endl;
      loadDemo(argv[i]);
      chosenDemo=1;
    } else if( strcmp(argv[i], "--savedemo") == 0)
    {
      i++;
      if(i > argc)
      {
...
int main(int argc, char **argv)
{
  ...
  cout << " F1 = start demo recording, F2 = stop demo recording and save to '"<<saveDemoFile<<"'" << endl;
  cout << "F10 toggle sound" << endl;
  cout << "Using level file '" << levelFile << "'" << endl;

  //Set initial resolution
  dispInfo.glBaseZoom=100.0;
